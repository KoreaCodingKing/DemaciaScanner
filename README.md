...
해당 결과는 {1,1,1,1....} 비동기 처리 안에서 map 함수의 속도보다 index의 값이 변경되는 속도가 느리기 때문임
(정확히는, 클로저 개념이 필요 -->
...
여기서 finally()에 if문을 사용하여 리스트.map((item, index))의 총 item 갯수와 index의 값이 같아지면
위 임시리스트(tempList)를 최종적으로 반환 하는 형태를 사용했다.
````
